# Data Loss Prevention (DLP) OBJ 4.4 
- setup to monitor the data of a system while it's in use, in transit, or at rest in order to detect any attempts to steal the data 
## Endpoint DLP System 
- piece of software that's installed on a workstation or a laptop, & it's going to monitor the data that's in use on that computer 
## Network DLP System
- piece of software or hardware that's a solution placed at the perimeter of the network to detect data in transit 
## Storage DLP
- software that's installed on a server in the data center & inspects the data while it's at rest on the server 
## Cloud-based DLP System
- usually offered as software-as-a-service (SaaS)
- part of the cloud service & storage needs 

---
# Configuring a DLP OBJ 4.5
